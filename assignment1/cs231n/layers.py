...
def affine_forward(x, w, b):
    ...
    x_new = x.reshape(num_inputs, total_dim)
...
def svm_loss(x, y):
    ...
    # num_inputs = x.shape[0]
    # correct_x = x[y]
    # score_gap = np.maximum(0, x - correct_x + 1)
    # score_gap[np.arange(num_inputs), y] = 0
    # loss = score_gap.sum() / num_inputs
...
def softmax_loss(x, y):
    ...
    # loss 부분
    # x -= np.max(x, axis=1).reshape(num_inputs, 1)
    total_scores_exp = np.sum(np.exp(x), axis=1).reshape(num_inputs, 1)
    softmax = np.exp(x) / total_scores_exp
